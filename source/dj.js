...
(function() {
     ...
     dj = function(selector) {
		var djob;
		if(selector.substr(0,1)=="#"){ // if id
			djob=document.getElementById(selector.substr(1));
		}else if(selector.substr(0,1)=="."){ //if class, slow in ie
			var djob=[];
			if(document.getElementsByClassName){
				var nodes=document.getElementsByClassName(selector.substr(1));
				for (var i = 0; i < nodes.length; i++) djob[i] = nodes[i];
			}else{ //if given tagname
				var nodes = document.getElementsByTagName("*");
				for(z=0;z<nodes.length;z++){
					if(nodes[z].getAttribute("class") == selector.substr(1))djob.push(nodes[z]);
				}
			}
		}else{
